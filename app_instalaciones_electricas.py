...
st.set_page_config(page_title="Estudio de Instalaciones Eléctricas", page_icon="⚡", layout="wide")

st.markdown("""
<style>
.main-title {font-size: 2.2rem; font-weight: 800; color: #17365D;}
.subtitle {font-size: 1.05rem; color: #555;}
.warning-box {padding: .9rem; border-radius: 10px; background-color: #fff3cd; border: 1px solid #ffeeba; color: #664d03;}
.ok-box {padding: .9rem; border-radius: 10px; background-color: #d1e7dd; border: 1px solid #badbcc; color: #0f5132;}
...
CONDUCTORES = [(15,"#14 AWG"),(20,"#12 AWG"),(30,"#10 AWG"),(40,"#8 AWG"),(55,"#6 AWG"),(70,"#4 AWG"),(85,"#3 AWG"),(100,"#2 AWG"),(115,"#1 AWG"),(130,"1/0 AWG"),(150,"2/0 AWG"),(175,"3/0 AWG"),(200,"4/0 AWG"),(230,"250 kcmil"),(255,"300 kcmil"),(285,"350 kcmil"),(310,"400 kcmil"),(335,"500 kcmil")]
AREA_MM2_APROX = {"#14 AWG":2.08,"#12 AWG":3.31,"#10 AWG":5.26,"#8 AWG":8.37,"#6 AWG":13.3,"#4 AWG":21.2,"#3 AWG":26.7,"#2 AWG":33.6,"#1 AWG":42.4,"1/0 AWG":53.5,"2/0 AWG":67.4,"3/0 AWG":85.0,"4/0 AWG":107.2,"250 kcmil":126.7,"300 kcmil":152.0,"350 kcmil":177.3,"400 kcmil":202.7,"500 kcmil":253.4}
...
def next_standard_breaker(current_a, factor=1.25):
    if pd.isna(current_a) or current_a <= 0: return np.nan
...
def conductor_sugerido(breaker):
    if pd.isna(breaker): return ""
    if isinstance(breaker, str): return "Revisar"
    for amp, cond in CONDUCTORES:
        if breaker <= amp: return cond
    return "Revisar"

def corriente(pot_total_w, voltaje, fases, fp):
    if voltaje <= 0 or fp <= 0: return np.nan
    if fases == 3: return pot_total_w / (math.sqrt(3) * voltaje * fp)
    return pot_total_w / (voltaje * fp)

def caida_tension_pct(corriente_a, distancia_m, voltaje, fases, conductor):
    if pd.isna(corriente_a) or corriente_a <= 0 or voltaje <= 0: return np.nan
    area = AREA_MM2_APROX.get(conductor)
    if not area: return np.nan
    rho = 0.0175
    if fases == 3:
...
def clasificar_observacion(row, limite_caida):
    obs = []
    if pd.notna(row.get("Caída de tensión (%)")) and row["Caída de tensión (%)"] > limite_caida: obs.append("Revisar caída de tensión")
    if row.get("Tipo de carga") == "No lineal": obs.append("Puede generar armónicos")
    if row.get("Tipo de carga") == "Inductiva": obs.append("Considerar corriente de arranque y FP")
    if pd.notna(row.get("FP")) and row["FP"] < 0.85: obs.append("Factor de potencia bajo")
...
def crear_excel(df_resultado, resumen, ups_gen):
    """Crea un .xlsx real sin openpyxl ni xlsxwriter."""
    output = BytesIO()
    sheets = {"Cargas Calculadas": df_resultado, "Resumen": pd.DataFrame([resumen]), "UPS_Generador": pd.DataFrame([ups_gen])}
    ...
    def sheet_xml(df):
        ...
        for c, col_name in enumerate(df.columns, start=1):
            ref = cell_ref(1, c)
            value = escape(str(col_name))
            header_cells.append(f'<c r="{ref}" t="inlineStr"><is><t>{value}</t></is></c>')
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>{"".join(rows)}</sheetData></worksheet>'''
    workbook_sheets, rels, content_types = "", "", ""
    for i, sheet_name in enumerate(sheets.keys(), start=1):
        safe_sheet_name = escape(sheet_name[:31])
        workbook_sheets += f'<sheet name="{safe_sheet_name}" sheetId="{i}" r:id="rId{i}"/>'
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>'''
    with ZipFile(output, "w", ZIP_DEFLATED) as z:
        ...
        for i, df in enumerate(sheets.values(), start=1):
            z.writestr(f"xl/worksheets/sheet{i}.xml", sheet_xml(df))
...
resumen = {"Cantidad de cargas": cantidad_cargas, "Potencia total (W)": round(float(pot_total),2), "Potencia total (kW)": round(float(pot_total/1000),2), "Corriente total aproximada (A)": round(float(corriente_total),2), "Potencia en UPS (W)": round(float(pot_ups),2), "Potencia en Generador (W)": round(float(pot_gen),2)}
...
ups_gen = {"Carga UPS (W)": round(float(pot_ups),2), "UPS recomendado (VA)": round(float(ups_va),2), "UPS recomendado (kVA)": round(float(ups_va/1000),2), "Autonomía requerida (min)": autonomia_min, "Energía de baterías estimada (Wh)": round(float(energia_baterias_wh),2), "Carga Generador (W)": round(float(pot_gen),2), "Generador recomendado (VA)": round(float(gen_va),2), "Generador recomendado (kVA)": round(float(gen_va/1000),2)}
...
with tabs[2]:
    ...
    st.dataframe(df_resultado.style.format({"Potencia total (W)":"{:,.0f}", "Corriente (A)":"{:,.2f}", "Caída de tensión (%)":"{:,.2f}", "FP":"{:.2f}"}), use_container_width=True)
...
with tabs[5]:
    ...
    st.download_button(label="📄 Descargar reporte en Excel", data=excel_bytes, file_name="reporte_instalacion_electrica.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
